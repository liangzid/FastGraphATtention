...
class LshTester():
    ...
    def run(self, type, k_vec = [2], l_vec = [2]):
        ...
        # matrix with the vector be found.
        ...
        for k in k_vec:
            ...
            for L in l_vec:
                ...
                accu=correct*1.0/(self.q_num*len(hits))

                print("{0}\t{1}\t{2}\t{3}".format(L, k, accu,float(lsh.get_avg_touched()) / len(self.datas)))
        ...
        for k in k_vec:
            ...
            for L in l_vec:
                ...
                for q, hits in zip(self.ext_queries, exact_hits):
                    ...
                    # print(len(hits))
                    # print(len(lsh_hits))
                    while len(lsh_hits) != len(hits):   # padding -1 if MIP-vector not be found.
                        lsh_hits.append(-1)
...
class LshTesterFactory():
    ...
    # type: l2 & cosine
    # mips: True for ALSH
    def createTester(qtype, mips, datas, queries, rand_num, num_neighbours):
        try:
            if False == mips:
            ...
            print("LshTesterFactory type error: " + qtype) 
            return
